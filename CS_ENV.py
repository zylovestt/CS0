...
class PROCESSOR:
    ...
    def cal_Aq(self):
        self.Aq=(self.sum_Aq+1)/(self.Exe+2)
...
class PROCESSORS:
    def __init__(self,pro_configs:list,avg_reward=False,pro_seq=True):
        self.num_pros=len(pro_configs)
        self.pros=[PROCESSOR(pro_config) for pro_config in pro_configs]
        if pro_seq:
            self.pros.sort(key=lambda x:x.pro_dic['er'])
    ...
    def __call__(self,tin:float,tasks:dict,action:np.ndarray,womiga:float,sigma:float):
        ...
        if task_time==0:
            print('ta_here!')
        dic={}
        dic['Q'],dic['T'],dic['C'],dic['F']=Q/num_tasks,task_time*womiga,cons,finish #change
        if self.avg_reward:
...
class JOB:
    def __init__(self,maxnum_tasks:int,task_configs:list,job_config:dict,task_seq:bool):
    ...
    def __call__(self):
        ...
        if self.task_seq:
            task_l=[(ez,rz) for ez,rz in zip(tasks['ez'],tasks['rz'])]
            task_l.sort(key=lambda x:x[0],reverse=True)
            tasks['ez']=[x[0] for x in task_l]
            tasks['rz']=[x[1] for x in task_l]
...
class CSENV:
    ...
    def __init__(self,pro_configs:list,maxnum_tasks:int,task_configs:list,job_config:dict,loc_config,
        ...
        state_beta=0.95,reward_one_type='std',train_init_seed=[],test_init_seed=[],set_one_cycles=10,
        pro_seq=True,task_seq=True):  #state_beta=1
        '''lams:Q,T,C,F'''
        if pro_seq and change_prob:
            print('pro_seq wrong!')
        self.name+=1
        ...
        self.job=JOB(self.maxnum_tasks,self.task_configs,self.job_config,task_seq)
        ...
        self.set_one_cycles=set_one_cycles
    ...
    def set_one(self):
        ...
        self.done=0
        self.num_steps=0
        if ((self.reward_one or self.state_one or self.set_break_time) and self.state_beta<1) or self.states_init or self.rewards_init or self.set_break_time_init:
            if (not self.reward_one) and (not self.state_one) and (not self.set_break_time) and self.state_beta==1:
                print('wrong state_beta!')
            print('set_one')
            ...
            for _ in range(self.set_one_cycles):
                state=self.send()
                while not self.done:
                    if self.state_one or self.states_init:
            ...
            if self.set_break_time_init:
                ...
                self.set_break_time_init=False
    ...
    def reset(self):
        ...
        self.processors=PROCESSORS(self.pro_configs,pro_seq=self.pro_seq)
        self.job.job_index=0
        self.job.tin=0
        if self.train:
