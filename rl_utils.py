...
def train_on_policy_agent_batch(env:CS_ENV.CSENV, agent, num_episodes,max_steps,cycles,T_cycles=torch.inf,T_max=0,print_steps=False,pre_epochs=100,device='cuda',reps=1e-1,change_optim=10000,lr=1e-4,step_base=1,save_name=None):
    ...
    while i_episode < num_episodes:
        ...
        while step<max_steps:
            ...
            #if env.reward_one:
            for key in env.tarb_dic:
                writer.add_scalar(tag='step_rewards_'+key,scalar_value=env.tarb_dic[key][-1]*env.lams[key[0]],global_step=all_steps)
                writer.add_scalar(tag='step_rewards_'+key[0],scalar_value=env.tar_dic[key[0]][-1]*env.lams[key[0]],global_step=all_steps)
        if done:
            '''print(env.bases)
            print(env.bases_fm)
            print(env.states_mean)
            print(env.states_std)
            print(env.time_break)'''
            return_list.append(episode_return)
            writer.add_scalar(tag='return',scalar_value=episode_return,global_step=i_episode)
            i_episode+=1
            if i_episode % cycles == 0:
